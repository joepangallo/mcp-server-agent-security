...
const dangerousEnvKeyPattern = /^(PATH|LD_PRELOAD|LD_LIBRARY_PATH|DYLD_INSERT_LIBRARIES|DYLD_LIBRARY_PATH|NODE_OPTIONS|PERL5OPT|RUBYOPT|PYTHONSTARTUP|PYTHONPATH|BROWSER|EDITOR|VISUAL|STARTUP_CMD|PROMPT_COMMAND)$/i;
const shellExecutionPattern = /\b(sh|bash|zsh|csh|tcsh|ksh|fish|dash|ash|cmd|powershell|pwsh|python|perl|ruby|node)\b/i;
...
function isHighRiskFilesystemScope(arg) {
  ...
  // Resolve .. traversals to detect /tmp/../etc → /etc
  const resolved = path.resolve("/", normalized);

  const dangerousPaths = ["/Users", "/home", "/root", "/etc", "/var", "/usr", "/bin", "/sbin", "/opt", "/proc", "/sys"];
  if (resolved === "/" || dangerousPaths.some((d) => resolved === d || resolved.startsWith(d + "/"))) {
    // Allow paths under /tmp that stay under /tmp after resolution
  ...
  return /^[A-Za-z]:$/.test(normalized) || /^[A-Za-z]:\\Users$/i.test(normalized);
}
...
function analyzeFilesystemScope(serverName, server, packageNames, findings) {
  ...
  const dangerousRoot = pathLikeArgs.some(isHighRiskFilesystemScope);
...
function analyzeShellExecution(serverName, server, findings) {
  ...
  if (/(^| )(sh|bash|zsh|csh|tcsh|ksh|fish|dash|ash|cmd|powershell|pwsh|perl|ruby)( |$)/i.test(command) && args.some((arg) => /^(-c|-e|\/c|-command)$/i.test(arg))) {
    findings.push(createFinding({
      source: "config-analyzer",
      severity: "critical",
      confidence: "high",
...
function analyzeEnvironment(serverName, server, findings) {
  ...
  for (const [key, value] of Object.entries(env)) {
    if (dangerousEnvKeyPattern.test(key)) {
      findings.push(createFinding({
        source: "config-analyzer",
        severity: "high",
        confidence: "high",
        ...
        description: `Dangerous environment variable "${key}" is set on MCP server "${serverName}". This variable can influence process execution, library loading, or code injection.`,
        remediation: "Remove execution-influencing environment variables (PATH, LD_PRELOAD, NODE_OPTIONS, etc.) from MCP configs. Let servers inherit only safe defaults."
      }));
      continue;
    }
  ...
  for (const [serverName, server] of servers) {
    try {
      ...
      findings.push(createFinding({
        ...
        cwe: "input_validation",
        location: serverName,
        description: `Failed to fully analyze MCP server "${serverName}".`,
