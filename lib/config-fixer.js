  ...
  for (const [name, server] of serverEntries) {
    ...
    if (/filesystem|file[-_ ]?system|server-filesystem|fs\b/i.test(corpus)) {
      if (Array.isArray(server.args)) {
        ...
        if (!hasPath || hasDangerousRoot) {
          changes.push({
            server: name,
            action: "constrain_filesystem",
            detail: hasDangerousRoot
              ? "Filesystem server has overly broad scope — replace with a specific project directory"
              : "Filesystem server has no path constraint — add an explicit allowed directory",
            severity: "high"
          });
        }
      }
