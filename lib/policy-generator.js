...
function classifyToolCapability(name, description, packageNames) {
  ...
  if (/(read|list|search|get|open|query|select|retrieve|filesystem|file|browser)/.test(corpus)) {
...
function generatePolicy(mcpConfig, options) {
  ...
  for (const [name, server] of servers) {
    ...
    const capabilities = classifyToolCapability(name, corpus, packageNames);
