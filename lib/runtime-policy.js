...
function findDisallowedRuntimeEnvKeys(envInput) {
  ...
  return Object.keys(envInput)
    .filter((key) => DISALLOWED_RUNTIME_ENV_KEYS.has(normalizeEnvKey(key)) || isDangerousEnvKey(key))
...
function stripDisallowedRuntimeEnvKeys(envInput) {
  ...
  for (const [key, value] of Object.entries(envInput)) {
    if (!DISALLOWED_RUNTIME_ENV_KEYS.has(normalizeEnvKey(key)) && !isDangerousEnvKey(key)) {
      filtered[key] = value;
    }
  }
